...
# def registerNext():
# 	#user = users(username=request.form["userid"],  password=request.form['loginpassword'])
...
# 	if password == confirm:
# 		#user = users(id = request.form['userid'],username = request.form['name'],password = secure_password)
# 		user = users(name = request.form['person_name'],username = request.form['name'],gender = request.form['gender'],email = request.form['email'],dob = request.form['dob'],govtid = request.form['govtid'],phoneNo = request.form['phoneNo'],password = password)
# 		print (user.password)
# 		db.session.add(user)
# 		db.session.commit()
...
def confirm_otp_next():
	...
	print "displayed"
	print otp_num
	if (int(otp_num) == int(entered_otp)) :
		print "otp verfied"
		flash('Your account has now been created.. You can now log in !!','success')
		return redirect(url_for('login'))
	else :
